...
Three things have to be measured rather than assumed, or the answer is wrong.
  ...
  sigma_eff       The reference Monte-Carlo uses ANTITHETIC pairs and an
                  ...
                  `stderr` is sqrt(Var(sample)/paths), which treats the
                  antithetic pairs as independent.  They are not, and the sign of
                  the error is not the one you would guess: on the RAW put the
                  pair correlation is about -0.58 (antithetic working as
                  intended), but the control variate subtracts a second, similar
                  put, and a DIFFERENCE of two puts is not monotone in the
                  driver — on the adjusted sample the pair correlation turns
                  POSITIVE (+0.11 to +0.19 over H in [0.05, 0.3]).  So the quoted
                  standard error is OPTIMISTIC by sqrt(1+rho_pair), 5-9% here.
                  sigma_eff is therefore computed as sd(sample)*sqrt(1+rho_pair)
                  from one large sample — exact up to across-pair independence,
                  which holds because the rows are drawn independently — and
                  cross-checked against the scatter of R independent
                  replications, an estimator that assumes nothing at all.
