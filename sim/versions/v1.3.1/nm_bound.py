...
#
# Calibrated by `runs/nm-bound-20260805T114838Z` (see ALGORITHMS.md, F081-F084
# and docs/N_M_BOUND.md).  The shape is the one the mechanism above predicts:
#
#     m*(n, H, eps)  ~=  A(H) * ln n * (1/eps)^p  +  B(H).
#
# Three things about these constants, all of which matter before anyone leans on
# them:
#
#   * p is FIXED at 1/2, not fitted.  Three tolerances on integer-valued data
#     cannot resolve it: on the `best` branch the free fit gave p between -0.21
#     and 0.81 across H, which is noise.  On `ajee` it is stable (0.43-0.62,
#     R^2 ~ 0.99), and 1/2 is what cell-wise moment matching predicts, so 1/2 is
#     the value the data actually supports.  The competing log(1/eps) shape is
#     NOT ruled out — the two are indistinguishable at three tolerances.
#   * calibrated on `ajee`, the implementable construction.  `ajee` needs more
#     factors than `best`, so as a guess for `best` this is conservative, which
#     is the safe direction.
#   * residuals reach about two factors.  This is a GUESS, not a bound;
#     `m_required` verifies it by computation before returning an m.
#
# A(H) falls with H — 0.246, 0.226, 0.191, 0.139 at H = 0.05, 0.1, 0.2, 0.3 —
# i.e. the rougher the model, the more factors each decade of grid refinement
# costs, which is the expected direction.
_LAW: dict[float, dict] = {}          # filled from the run; see set_law()
_LAW_SOURCE = "uncalibrated"

_MEASURED_LAW = {
    0.05: {"A": 0.24623, "p": 0.5, "B": 0.3627},
    0.10: {"A": 0.22579, "p": 0.5, "B": 0.4328},
    0.20: {"A": 0.19066, "p": 0.5, "B": 0.3456},
    0.30: {"A": 0.13884, "p": 0.5, "B": 0.4441},
}
...
set_law(_MEASURED_LAW, "runs/nm-bound-20260805T114838Z (ajee, p fixed at 1/2)")
...
def m_required(n: int, H: float, tol: float = 0.03, T: float = 1.0,
    ...
    With `verify` (the default) the fitted law is not trusted at all: the whole
    error curve is computed and the smallest m actually meeting `tol` is
    returned, with `guess_ok` recording whether the law agreed.  So the answer is
    a measurement, and the law is only a reported guess.  `verify=False` returns
    the law's guess alone — cheap, and it can be wrong.
...
# ------------------------------------------------------- ready-to-use in a run
def lift_for_tolerance(n: int, H: float, tol: float = 0.03, T: float = 1.0,
                       construction: str = "best") -> dict:
    """A lift sized by the criterion, in the normalisation the LATTICE uses.

    One call for a runner that should not be choosing m by hand: pick the
    smallest m meeting `tol`, and return its (w, s) already scaled for
    K(u) = sqrt(2H) u^h, which is the rough Bergomi convention of
    `route_aprime.py` / `route_b_lattice.py` (route_b.py optimises for
    u^h/Gamma(1+h); the two differ by the constant sqrt(2H) Gamma(1+h), and both
    K and K^m are linear in the weights, so rescaling the weights is exact).

    On the `best` branch this reproduces `route_b_lattice.lift_for(n, m, H)`
    exactly — same optimiser, same warm-start chain, same t0 = delta — so a
    runner can size m here and hand the result straight to `LiftedLattice`
    without changing what the lattice computes.  `verify/verify_claims.py`
    checks that agreement.
    """
    h = H - 0.5
    delta = T / n
    info = m_required(n, H, tol, T, construction=construction)
    m = info["m"]
    if construction == "geometric":
        w, s, _ = geometric_lift(m, H, n, T)
    else:
    ...
    return {"m": m, "w": w * scale, "s": s,
            "rel_error": info.get("rel_error"), "tol": tol,
            "construction": construction, "law_source": _LAW_SOURCE}
