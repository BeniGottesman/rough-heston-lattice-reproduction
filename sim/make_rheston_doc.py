...
def main() -> None:
    ...
    try:
        cr = latest("cost-equivalence")
        cmd, cres = load(cr)
    except SystemExit:
        cr, cmd, cres = None, None, {}
    ...
    if cmd is not None:
        P.append("\n---\n")
        P.append("# Part VII — what does a lattice price actually cost?\n")
        P.append("Every table above compares NUMBERS. This one compares "
                 "PRICE-PER-SECOND, which is the question a user asks once the "
                 "numbers are settled: a lattice run at `n = 16` takes some "
                 "wall time and lands some distance from the truth — how many "
                 "Monte-Carlo paths, and how much time, does it take to land "
                 "the same distance away?\n")
        P.append("The comparison is only meaningful once the accuracies are "
                 "matched, because the two errors are different animals. The "
                 "lattice's error is a **bias**: deterministic, and it does not "
                 "shrink if you run it again. The Monte-Carlo's is a "
                 "**standard error**: random, and it shrinks like `1/√N`. So "
                 "the equivalence is\n")
        P.append("```\n    N*  =  (σ_eff / |lattice bias|)²\n```\n")
        P.append("and then the two wall times can be set side by side.\n")
        P.append("Three quantities are measured rather than assumed, because "
                 "each of them would otherwise silently break the answer. "
                 "`σ_eff` is measured from the **scatter of independent "
                 "replications**, not from a formula — the reference "
                 "Monte-Carlo uses antithetic pairs and an `η = 0` control "
                 "variate, and its own `stderr` treats the pairs as "
                 "independent. The Monte-Carlo's **one-off setup** (an "
                 "`nfine × nfine` covariance factorisation) is timed separately "
                 "from its marginal path cost, since at small `N` the setup is "
                 "the whole run. And the **reference's own standard error** is "
                 "propagated into `N*`, so a bias that is not resolved reports "
                 "a band instead of a number.\n")
        P.append("The one thing to hold on to before reading the numbers: this "
                 "compares **these implementations**, not the two methods. The "
                 "Monte-Carlo's inner loop is a multi-threaded BLAS matrix "
                 "product; the lattices are single-threaded Python loops over a "
                 "state space, with no C++ port for Route B. A constant factor "
                 "of tens is implementation, not mathematics — the algorithmic "
                 "scalings are given in the run's own section 6 so a reader can "
                 "rescale.\n")
        P.append("One column needs reading with care before the table below is "
                 "used as an argument. The run reports the **American** price "
                 "and its cost, on the grounds that this is the thing "
                 "Monte-Carlo cannot match cheaply. But this rough Bergomi "
                 "convention carries **no interest rate**, and with `r = 0` a "
                 "put has essentially no reason to be exercised early — the "
                 "measured premium is `+0.0003` to `+0.0006`, i.e. zero to "
                 "within the scheme's own error. So that table **validates the "
                 "American machinery** (it correctly finds almost nothing) "
                 "rather than demonstrating its value. The lattice's "
                 "early-exercise advantage is real but it is not evidenced "
                 "here; showing it needs `r > 0`, which the rough Heston side of "
                 "the project has and this one does not.\n")
        P.append(f"*Generated by `sim/run_cost_equivalence.py`, run "
                 f"`{cr.name}`.*\n")
        P.append(body(cmd))
    ...
    P.append("- **Control variate** — a quantity simulated alongside the payoff "
             ...
             "error.\n")
    if cmd is not None:
        P.append("- **`N*` (equal-accuracy path count)** — how many simulated "
                 "paths the Monte-Carlo needs before its statistical "
                 "uncertainty is as small as the lattice's fixed error. The "
                 "number that makes a speed comparison fair.\n")
        P.append("- **`σ_eff`** — how much a single simulated path varies. It "
                 "sets how fast the Monte-Carlo's error shrinks, and everything "
                 "in `N*` depends on it, which is why it is measured from the "
                 "spread of repeated runs rather than taken from a formula.\n")
        P.append("- **Setup cost** — a one-off preparation the Monte-Carlo pays "
                 "before its first path (factorising the driver's covariance). "
                 "Paid once per run, not per path, so it dominates a small run "
                 "and disappears in a large one.\n")
        P.append("- **Break-even error** — the accuracy the Monte-Carlo reaches "
                 "if you give it exactly the lattice's wall time. If it is "
                 "smaller than the lattice's error, simulation was the cheaper "
