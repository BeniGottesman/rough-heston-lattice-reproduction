...
A low `R2` on a line means the error changes sign inside the ladder, so the small error at large `n` is partly cancellation. The binomial's order is dragged below one wherever the variance deficit `mu^2 delta` dominates; the trinomial, which has no such deficit, sits at order one throughout. Section 10.8.3 fits `0.99` on a different parameter set, so order one is not an artefact of one choice.
...
|---|---|---|---|---|
| binomial | 50 | 10 | 0.85s | 5,501 |
| binomial | 100 | 15 | 10.15s | 16,401 |
| binomial | 200 | 20 | 153.54s | 43,601 |
| trinomial | 50 | 10 | 2.28s | 7,101 |
| trinomial | 100 | 15 | 18.74s | 21,201 |
| trinomial | 200 | 20 | 196.04s | 56,001 |

## Compute time

| phase | wall time | seconds |
|---|---|---|
| 1. 1. the reference, and its own convergence | 0s | 0.36 |
| **total** | **6m21s** | **381.98** |
